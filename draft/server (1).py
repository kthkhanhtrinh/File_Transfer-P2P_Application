...
def server_recv(conn, ip): #publish/fetch
    while True:
        try:
            ...
            if data.startswith("fetch"):
                _, fname, c1 = data.split("_")
                
                peer_ip = search_fname(conn, fname) #c2

                #check c2 still alive
                if not checkOnline(peer_ip):
                    conn.send(f"fetch fail_{fname}".encode()) 
                    continue
                
                #if c2 is alive → send c2
                conn.send(f"fetch-request_{fname}_{c1}".encode())
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
                for line in file:
                    if fname in line:
                        ...
                        print(peer_ip) #c2
                        #
                        # print(f"String found in file: {os.path.basename(file_name)}, line: {line} ***")
                        # server_send_file(conn, file_path, line)
                        return peer_ip
